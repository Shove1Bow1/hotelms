...
$('#booking').submit(function () {
    ...
    if(!room_no && !first_name && !contact_no && !id_card_no && !address){
        $('.response').html('<div class="alert bg-danger alert-dismissable" role="alert"><em class="fa fa-lg fa-warning">&nbsp;</em>Vui Lòng Điền Đầy Đủ Các Thông tin Cần Thiết</div>');
    }else{
        $.ajax({
            type: 'post',
            url: 'ajax.php',
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.done == true){
                ...
                $('.emp-response').html('<div class="alert bg-success alert-dismissable" role="alert"><em class="fa fa-lg fa-warning">&nbsp;</em>Thêm nhân viên mới thành công</div>');
            }else{
